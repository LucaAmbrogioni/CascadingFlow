...
value = join_link(true_smpl[-1][0,-1], true_smpl[-1][0,-2]).detach().numpy() + np.random.normal(0,sigma)
print(value)
...
transformations = [TriResNet(d_x=1, d_epsilon=d_eps, epsilon_nu=0.0001, in_pre_lambda=1., scale_w=0.8,) for _ in range(2**depth-1)]
...
for p in parames_list:
    params += p
optimizer = optim.Adam(params, lr=0.00025)
...
### Global flow ###
post_model_gf = GlobalFlow(T=2**depth-2, d_x=1, d_eps=20)
loss_list4 = []
...
for itr in tqdm(range(num_iterations)):
    ...
    # Variational loss
    x, samples_pre, _, log_jacobian, epsilon_loss = post_model_gf.sample_timeseries(batch_size)
...
plt.scatter((smpl[:,-1]), (smpl[:,-2]), c="r", alpha=0.01)
...
plt.hist(join_link(smpl[:,-1],smpl[:,-2]),30, alpha=0.5, color="r")
