...
def evaluate_img_model(variational_model, X_true, M, emission_model, emission_distribution, scale, out_data, T_data):
  X, mu, x_pre, log_jacobian, epsilon_loss = variational_model.sample_timeseries(M)
  uni_lk = evaluate_img_likelihood(X.detach().numpy(), X_true.detach().numpy())
  #multi_lk = evaluate_multi_img_likelihood(X.detach().numpy(), X_true.detach().numpy())
  multi_pred_lk = evaluate_img_predictive_error(X, emission_model, emission_distribution, scale, out_data, T_data)
  print("Avarage univariate latent likelihood: {}".format(uni_lk))
  print("Multivariate latent likelihood: {}".format(0))
  print("Predictive observable likelihood: {}".format(multi_pred_lk))
  # todo: fix multi_lk
  return uni_lk, 0, multi_pred_lk

def evaluate_img_likelihood(X, x_true, s=0.00001):
  def ker(x,y,bw):
      return np.mean(np.log(np.mean(np.exp(-(x - y)**2/(2*bw**2))/(np.sqrt(2*np.pi)*bw),0) + s))
...
def evaluate_multi_img_likelihood(X, x_true, s=0.01):
  try:
    if len(X.shape) == 3:
      a,b,c = X.shape
      N = b*c
...
def evaluate_img_predictive_error(X, emission_model, emission_distribution, scale, out_data, T_data):
  ...
  for t in range(T_data, X.shape[-1]):
    ...
    if scale is None:
      Y[:,:,tau] = emission_distribution.rsample(emission_model(X[:,:,:,:,t]).squeeze(), None).detach().numpy() + eps
    else:
      Y[:,:,tau] = emission_distribution.rsample(emission_model(X[:,:,:,:,t]).squeeze(), scale).detach().numpy() + eps
  #plt.plot(np.transpose(Y), alpha=0.01, c="r")
  #plt.plot(np.transpose(X[:20,0,T_data:].detach().numpy()), alpha=0.8, c="k")
  #plt.plot(np.transpose(out_data))
  #plt.show()
